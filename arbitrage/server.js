...
function recordKraken(data){
 ...
 if(history.length>500)history.shift();totalScans++;if(candidate)candidateCount++;
}
async function cached(key,fn){
 const old=cache.get(key);if(old&&Date.now()-old.time<60000)return old.data;
...
const server=http.createServer((req,res)=>{
 ...
 // Legacy n8n HTTP endpoint retained so its schedule does not fail; never orders.
 if(req.method==='GET'&&req.url==='/kraken'){cached('kraken',krakenScan).then(d=>send(res,200,{...d,legacy:true,activeStrategy:false})).catch(e=>send(res,503,{mode:'PAPER',source:'Kraken',error:String(e.message).slice(0,140),tradesEnabled:false}));return;}
