...
let totalScans = 0, candidateCount = 0;
...
async function backgroundScan() {try {await cached('kraken',krakenScan); console.log('PAPER autoscan succeeded; totalScans='+totalScans+'; orders disabled');} catch(e) {console.error('PAPER autoscan error: '+String(e.message).slice(0,140));}}
...
if(require.main===module) server.listen(port,'0.0.0.0',()=>{
  console.log('ANTON Arbitrage PAPER dashboard on '+port+'; public Kraken scan; real trading disabled; background='+(process.env.PAPER_AUTOSCAN==='true'));
