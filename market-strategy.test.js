...
const {compute,chooseSignal}=require('./market-strategy');
...
function fixture({ethStale=false,oi='1000000',funding='0',vol=100,rise=false}={}){
  ...
  const derivativeInstId='BTC-USDT-SWAP';
  return {btc:{code:'0',data:rows(67000)},eth:{code:'0',data:rows(3200,ethStale)},derivativeInstId,oi:{code:'0',data:[{instId:derivativeInstId,oiUsd:oi,ts:String(time)}]},funding:{code:'0',data:[{instId:derivativeInstId,fundingRate:funding}]}};
}
...
test('fail closed on missing or mismatched derivative data',()=>{let x=compute(fixture({oi:null}),{},time);assert.equal(x.actionable,false);assert.equal(x.reasons[0],'DERIVATIVES_DATA_UNAVAILABLE');let m=fixture();m.oi.data[0].instId='ETH-USDT-SWAP';x=compute(m,{},time);assert.equal(x.actionable,false);assert.equal(x.reasons[0],'DERIVATIVES_INSTRUMENT_MISMATCH')});
