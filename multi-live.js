...
const MAX_ORDER=Math.min(20,Number(process.env.MAX_ORDER_EUR||5));
...
const ACCOUNT_BASELINE_EUR=300;
...
async function portfolio(){if(Date.now()-BOT_START>=85*86400_000)throw Error('ORDER_ARCHIVE_TIME_HORIZON_EXCEEDED');const bal=await signer('GET','/api/v5/account/balance?ccy=EUR,BTC,ETH,DOGE');const details=bal[0]?.details;if(!Array.isArray(details))throw Error('ACCOUNT_BALANCE_UNVERIFIED');const eurRow=details.find(x=>x.ccy==='EUR');const availableEur=eurRow?n(eurRow.availBal):0;if(!(availableEur>=0))throw Error('AVAILABLE_EUR_UNVERIFIED');const balances=Object.fromEntries(details.map(x=>[x.ccy,n(x.availBal)]));
  const totalBalances=Object.fromEntries(details.map(x=>[x.ccy,n(x.cashBal)]));const pairs=[];for(const pair of PAIRS){const [recent,archive,pending,instruments,ticker]=await Promise.all([signer('GET',`/api/v5/trade/orders-history?instType=SPOT&instId=${pair}&limit=100`),signer('GET',`/api/v5/trade/orders-history-archive?instType=SPOT&instId=${pair}&limit=100`),signer('GET',`/api/v5/trade/orders-pending?instType=SPOT&instId=${pair}&limit=100`),signer('GET',`/api/v5/public/instruments?instType=SPOT&instId=${pair}`),publicGet(`/api/v5/market/ticker?instId=${pair}`)]);if(pending.length>=100||!instruments[0]||!ticker.data[0])throw Error('PAIR_UNVERIFIED_'+pair);const price=requirePositive(ticker.data[0].last,'TICKER_'+pair);const orders=uniqueOrders(recent,archive);const book=ledger(orders,price);const report=reportLedger(orders,price,pair);const free=balances[pair.split('-')[0]]??0;if(!(free>=0))throw Error('BASE_BALANCE_UNVERIFIED_'+pair);pairs.push({pair,price,...book,pnlEur:report.pnlEur,free,pending:pending.filter(x=>String(x.clOrdId||'').startsWith('ANTON')).length,instrument:instruments[0],minTrade:round(Number(instruments[0].minSz)*price*1.03)});}const exposureEur=pairs.reduce((s,p)=>s+p.exposure,0),pnlEur=pairs.reduce((s,p)=>s+p.pnlEur,0);if(!Number.isFinite(exposureEur)||!Number.isFinite(pnlEur))throw Error('PORTFOLIO_UNVERIFIED');
  const equityEur=(Number.isFinite(totalBalances.EUR)?totalBalances.EUR:0)+pairs.reduce((s,p)=>s+(Number.isFinite(totalBalances[p.pair.split('-')[0]])?totalBalances[p.pair.split('-')[0]]:0)*p.price,0);
  if(!Number.isFinite(equityEur)||equityEur<0)throw Error('ACCOUNT_EQUITY_UNVERIFIED');
  const accountPnlEur=equityEur-ACCOUNT_BASELINE_EUR;
  return{pairs,exposureEur,pnlEur,equityEur,accountPnlEur,accountBaselineEur:ACCOUNT_BASELINE_EUR,availableEur,filledOrders:pairs.reduce((s,p)=>s+p.fills,0),mode:LIVE?'LIVE':'MONITOR_ONLY',updatedAt:new Date().toISOString(),capitalLimitEur:CAP_EUR,orderLimitEur:MAX_ORDER};}
async function dashboardFallback(cause){
  ...
  return {
    ...
    diagnostic:cause,mode:LIVE?'LIVE — SAFE HOLD':'MONITOR_ONLY',
