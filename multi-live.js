...
function parsePnlBaselines(raw){
  ...
  let value;
  try{value=JSON.parse(raw);}catch{throw Error('INVALID_PNL_BASELINES_JSON');}
  if(!value||Array.isArray(value)||typeof value!=='object')throw Error('INVALID_PNL_BASELINES_JSON');
  const out={};
  for(const key of Object.keys(value)){
    if(!PAIRS.includes(key))throw Error('UNKNOWN_PNL_BASELINE_PAIR');
    const amount=Number(value[key]);
    if(!Number.isFinite(amount))throw Error('INVALID_PNL_BASELINE_VALUE');
    out[key]=amount;
  }
...
function approvalAuthorized(req){return Boolean(process.env.SIGNER_TOKEN)&&safeEqual(req.headers.authorization,'Bearer '+process.env.SIGNER_TOKEN);}
async function readJson(req){let raw='';for await(const chunk of req){raw+=chunk.toString('utf8');if(Buffer.byteLength(raw)>4096)throw Error('REQUEST_TOO_LARGE');}const value=JSON.parse(raw||'{}');if(!value||Array.isArray(value)||typeof value!=='object')throw Error('INVALID_JSON');return value;}
...
function queueProposal({pair,side,reason,price,size,valueEur},now=Date.now()){
  ...
  const proposal={id:crypto.randomBytes(9).toString('base64url'),pair,side,reason,price:Number(price),size:String(size),valueEur:Number(valueEur),createdAt:now,expiresAt:now+PROPOSAL_TTL_MS,used:false};
  if(!PAIRS.includes(pair)||!['buy','sell'].includes(side)||!(proposal.price>0)||!(proposal.valueEur>0))throw Error('INVALID_PROPOSAL');
  pendingProposal=proposal;
  console.log('ANTON_TRADE_PROPOSAL '+JSON.stringify(publicProposal(proposal)));
  return proposal;
}
...
function applyPnlBaseline(book,pair,baselines=PNL_BASELINES){
  const baseline=Number(baselines[pair]||0);
  if(!Number.isFinite(baseline))throw Error('INVALID_PNL_BASELINE_VALUE');
  return {...book,pnlEur:book.pnlEur-baseline};
}
...
async function dashboardFallback(cause){
  ...
  if(!Array.isArray(details))throw Error('ACCOUNT_BALANCE_UNVERIFIED');
  const balances=Object.fromEntries(details.map(x=>[x.ccy,n(x.availBal)]));
  const availableEur=Number.isFinite(balances.EUR)?balances.EUR:0;
  const pairs=[];
  for(const pair of PAIRS){
    const [instruments,ticker]=await Promise.all([
      signer('GET',`/api/v5/public/instruments?instType=SPOT&instId=${pair}`),
      publicGet(`/api/v5/market/ticker?instId=${pair}`)
    ]);
    if(!instruments[0]||!ticker.data[0])throw Error('PAIR_UNVERIFIED_'+pair);
    const price=requirePositive(ticker.data[0].last,'TICKER_'+pair);
    const qty=balances[pair.split('-')[0]];
    if(!(qty>=0))throw Error('BASE_BALANCE_UNVERIFIED_'+pair);
    pairs.push({
      pair,price,qty,exposure:qty*price,pnlEur:null,
      minTrade:round(Number(instruments[0].minSz)*price*1.03),
      instrumentLive:instruments[0].state==='live'
    });
  }
  return {
...
async function executeApprovedProposal(id){
  const proposal=activeProposal();
  if(!proposal||proposal.id!==String(id||''))throw Error('PROPOSAL_NOT_ACTIVE');
  if(busy)throw Error('COORDINATOR_BUSY');
  busy=true;
  try{
    const book=await portfolio();
    if(book.pairs.some(p=>p.pending))throw Error('BOT_ORDER_PENDING');
    const p=book.pairs.find(x=>x.pair===proposal.pair);
    if(!p)throw Error('PAIR_NOT_FOUND');
    const drift=Math.abs(p.price/proposal.price-1);
    if(!Number.isFinite(drift)||drift>MAX_APPROVAL_PRICE_DRIFT)throw Error('PROPOSAL_PRICE_MOVED');
    let size;
    if(proposal.side==='buy'){
      const blocked=entryBlock(p,true);
      if(blocked)throw Error('BUY_BLOCKED_'+blocked);
      ...
      if(!reason||reason!==proposal.reason)throw Error('SELL_CONDITION_NO_LONGER_VALID');
      size=sellSize(p.qty,p.free,p.instrument.lotSz,p.instrument.minSz);
      if(!size)throw Error('SELL_SIZE_UNAVAILABLE');
    }
    const orderId='ANTON'+crypto.randomBytes(10).toString('hex');
    const result=await submit(p.pair,proposal.side,size,orderId);
    proposal.used=true;pendingProposal=null;cache=null;
    if(proposal.side==='buy'){const state=states.get(p.pair);state.lastSignal='BUY';state.lastSignalAt=Date.now();}
    console.log('ANTON_APPROVED_TRADE '+JSON.stringify({proposalId:proposal.id,pair:p.pair,side:proposal.side,orderId:result.ordId}));
    return{pair:p.pair,side:proposal.side,orderId:result.ordId,size,price:p.price};
  }finally{busy=false;}
}
async function tick(){
  if(busy||uncertain||(!LIVE&&!APPROVAL_MODE))return;
  busy=true;
  try{
...
if(route==='/trade-approval'&&req.method==='POST'){
  ...
  try{
    ...
    if(body.decision==='reject'){proposal.used=true;pendingProposal=null;console.log('ANTON_TRADE_REJECTED '+proposal.id);return json(res,200,{ok:true,rejected:true});}
    if(body.decision!=='approve')return json(res,400,{ok:false,error:'INVALID_DECISION'});
    const result=await executeApprovedProposal(body.id);
    return json(res,200,{ok:true,executed:true,...result});
  }catch(e){console.error('ANTON_APPROVAL_ERROR '+e.message);return json(res,409,{ok:false,error:e.message});}
}
if(req.method==='GET'&&(route==='/dashboard'||route==='/multi-data')){let auth;try{auth=await checkAuth(req,'/dashboard'+(route==='/dashboard'?(req.url.slice(route.length)||''):''));}catch{return json(res,502,{ok:false,error:'Auth unavailable'});}if(auth.status!==200){res.writeHead(auth.status||502,{...auth.headers,'cache-control':'no-store'});return res.end(auth.text);}if(route==='/dashboard'){res.writeHead(200,{'content-type':'text/html; charset=utf-8','cache-control':'no-store','x-frame-options':'DENY','referrer-policy':'no-referrer'});return res.end(page());}try{if(!cache||Date.now()-cache.at>10000)cache={at:Date.now(),value:await portfolio()};return json(res,200,{ok:true,...cache.value,pairs:cache.value.pairs.map(p=>({pair:p.pair,price:p.price,qty:p.qty,exposure:round(p.exposure),pnlEur:round(p.pnlEur),minTrade:p.minTrade,instrumentLive:p.instrument.state==='live'}))});}catch(e){console.error('ANTON_MULTI_DASHBOARD_ERROR '+e.message);try{return json(res,200,await dashboardFallback(e.message));}catch(f){console.error('ANTON_MULTI_DASHBOARD_FALLBACK_ERROR '+f.message);return json(res,502,{ok:false,error:'Данные OKX временно недоступны'});}}}return proxy(req,res);}
function run(){if(!Number.isInteger(PORT)||PORT<1||PORT>65500||!Number.isFinite(MAX_ORDER)||!(MAX_ORDER>0)||MAX_ORDER>5||!Number.isFinite(CAP_EUR)||CAP_EUR<1||CAP_EUR>1000||(APPROVAL_MODE&&!process.env.SIGNER_TOKEN))throw Error('UNSAFE_CONFIGURATION');const child=spawn(process.execPath,[path.join(__dirname,'dashboard-launcher.js')],{env:{...process.env,PORT:String(CHILD_PORT),MARKET_ONLY_LIVE:'false'},stdio:'inherit'});const server=http.createServer((req,res)=>{handler(req,res).catch(e=>{console.error('ANTON_MULTI_HTTP_ERROR '+e.message);if(!res.headersSent)json(res,502,{ok:false,error:'Unavailable'});else res.destroy();});});server.listen(PORT,'0.0.0.0',()=>console.log('ANTON_MULTI_READY '+JSON.stringify({pairs:PAIRS,live:LIVE,maxOrderEur:MAX_ORDER,capEur:CAP_EUR,signalIntervalMinutes:SIGNAL_PERIOD/60000,allowDustReentry:ALLOW_DUST_REENTRY})));const initial=setTimeout(tick,60000),interval=setInterval(tick,SIGNAL_PERIOD);let closing=false;const stop=s=>{if(closing)return;closing=true;clearTimeout(initial);clearInterval(interval);server.close();child.kill(s);};process.once('SIGTERM',()=>stop('SIGTERM'));process.once('SIGINT',()=>stop('SIGINT'));child.once('exit',(code,s)=>{clearTimeout(initial);clearInterval(interval);server.close();if(!closing)console.error('ANTON_CHILD_EXIT '+JSON.stringify({code,s}));process.exit(code==null?1:code);});}
if(require.main===module)run();module.exports={PAIRS,ledger,reportLedger,applyPnlBaseline,parsePnlBaselines,uniqueOrders,orderSize,sellSize,page,handler,tick,portfolio,activeProposal,publicProposal,queueProposal,executeApprovedProposal};
