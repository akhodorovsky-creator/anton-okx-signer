...
function maxOrderEur(raw){const value=Number(raw);if(raw===undefined||raw===''||!Number.isFinite(value)||value<1||value>20)throw Error('INVALID_MAX_ORDER_EUR');return value;}
const MAX_ORDER=maxOrderEur(process.env.MAX_ORDER_EUR);
  ...
  try{
    ...
    const [btc,eth,doge,derivativeRows]=await Promise.all([
      ...
      Promise.all(PAIRS.map(async pair=>{
        const instId=DERIVATIVE_INSTRUMENTS[pair];
        ...
        return {pair,instId,oi,funding};
      }))
    ]);
    const feeds={'BTC-EUR':btc,'ETH-EUR':eth,'DOGE-EUR':doge};
    const derivatives=Object.fromEntries(derivativeRows.map(row=>[row.pair,row]));
    for(const p of book.pairs){
      const state=states.get(p.pair), now=Date.now(), derivative=derivatives[p.pair];
      const fast=SIGNAL_PERIOD<PERIOD;
      const window=fast?observeOi(state,derivative.oi.code==='0'?derivative.oi.data[0]:null,now,derivative.instId):null;
      const computeState=fast?{...state,lastOi:window.ready?window.prior:null}:state;
      const d=compute({btc:feeds[p.pair],eth:p.pair==='BTC-EUR'?eth:btc,oi:derivative.oi,funding:derivative.funding,derivativeInstId:derivative.instId},computeState,now);
      ...
      const oiNow=n(derivative.oi.data[0]?.oiUsd??derivative.oi.data[0]?.oi);
...
if(require.main===module)run();module.exports={PAIRS,DERIVATIVE_INSTRUMENTS,ledger,reportLedger,applyPnlBaseline,parsePnlBaselines,uniqueOrders,orderSize,sellSize,page,handler,tick,portfolio,maxOrderEur};
