...
- Flow inputs are pair-scoped: BTC-EUR uses BTC-USDT-SWAP, ETH-EUR uses ETH-USDT-SWAP, DOGE-EUR uses DOGE-USDT-SWAP for open interest and funding. Instrument IDs are validated fail-closed before flow can contribute to a signal.
...
1. package.json
2. multi-live.js
3. market-strategy.js
4. frequency-policy.js
5. trading-guard-source-check.js
6. eur-order-cap-launcher.js (compatibility validator only; no runtime patching)
7. lot-size.js
8. safety-gateway.js
9. server.js
10. risk-monitor.js
11. dashboard-launcher.js
12. telegram-bot.js
...
- LIVE_ENABLED
- MARKET_ONLY_LIVE
- MAX_ORDER_EUR (required by multi-live.js; numeric range 1–20 EUR, invalid/missing value fails closed)
- MAX_ORDER_USDT
...
## Runtime structure after hardening
- npm start executes multi-live.js directly; eur-order-cap-launcher.js no longer rewrites source in memory.
- The former launcher's legacy /auto block and reconciled-dust behavior are native code in multi-live.js.
- .github/workflows/safety-checks.yml runs trading-guard-source-check.js in addition to npm run check and npm test.
- The static guard check intentionally fails if key order-cap, capital-cap, pair list, TP/SL, legacy /auto, dust or entry guards disappear.
- Rollback branch created before these changes: rollback/autonomous-live-trading-20260924-pre-hardening.
